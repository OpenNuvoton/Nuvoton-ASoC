		...
		int cmd, struct snd_soc_dai *dai)
{
	...
	struct nau8315_priv *nau8315 =	snd_soc_codec_get_drvdata(codec);
	...
	case SNDRV_PCM_TRIGGER_PAUSE_RELEASE:
		if (nau8315->enpin_switch) {
			gpio_set_value(nau8315->enable, 1);
			dev_dbg(codec->dev, "set enable to 1");
		}
		break;
	case SNDRV_PCM_TRIGGER_STOP:
	case SNDRV_PCM_TRIGGER_SUSPEND:
	case SNDRV_PCM_TRIGGER_PAUSE_PUSH:
		gpio_set_value(nau8315->enable, 0);
		dev_dbg(codec->dev, "set enable to 0");
		break;
	}
...
static const struct snd_soc_codec_driver nau8315_codec_driver = {
...
static int nau8315_platform_probe(struct platform_device *pdev)
{
	...
	nau8315 = devm_kzalloc(dev, sizeof(*nau8315), GFP_KERNEL);
	if (!nau8315)
		return -ENOMEM;

	nau8315->enable = -EINVAL;

	if (of_match_device(of_match_ptr(nau8315_device_id), dev))
		nau8315->enable = of_get_named_gpio(dev->of_node,
							"enable-gpio", 0);

	if (gpio_is_valid(nau8315->enable))
		if (devm_gpio_request_one(dev, nau8315->enable,
