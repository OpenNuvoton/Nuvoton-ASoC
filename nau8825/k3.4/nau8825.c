...
{
	struct nau8825 *nau8825 = snd_soc_codec_get_drvdata(w->codec);
...
static int nau8825_mclk_prepare(struct nau8825 *nau8825, unsigned int freq)
{
	...
	nau8825->mclk = clk_get(nau8825->dev, "mclk");
...
static int nau8825_read_device_properties(struct device *dev,
	...
	nau8825->jkdet_enable = of_property_read_bool(dev->of_node,
		"nuvoton,jkdet-enable");
	nau8825->jkdet_pull_enable = of_property_read_bool(dev->of_node,
		"nuvoton,jkdet-pull-enable");
	nau8825->jkdet_pull_up = of_property_read_bool(dev->of_node,
		"nuvoton,jkdet-pull-up");
	ret = of_property_read_u32(dev->of_node, "nuvoton,jkdet-polarity",
		&nau8825->jkdet_polarity);
	if (ret)
		nau8825->jkdet_polarity = 1;
	ret = of_property_read_u32(dev->of_node, "nuvoton,micbias-voltage",
		&nau8825->micbias_voltage);
	if (ret)
	...
	ret = of_property_read_u32(dev->of_node, "nuvoton,sar-threshold-num",
		&nau8825->sar_threshold_num);
	if (ret)
		nau8825->sar_threshold_num = 4;
	ret = of_property_read_u32_array(dev->of_node, "nuvoton,sar-threshold",
	...
	ret = of_property_read_u32(dev->of_node, "nuvoton,sar-hysteresis",
		&nau8825->sar_hysteresis);
	if (ret)
	...
	ret = of_property_read_u32(dev->of_node, "nuvoton,short-key-debounce",
		&nau8825->key_debounce);
	if (ret)
		nau8825->key_debounce = 3;
	ret = of_property_read_u32(dev->of_node, "nuvoton,jack-insert-debounce",
		&nau8825->jack_insert_debounce);
	if (ret)
		nau8825->jack_insert_debounce = 7;
	ret = of_property_read_u32(dev->of_node, "nuvoton,jack-eject-debounce",
		&nau8825->jack_eject_debounce);
	if (ret)
		nau8825->jack_eject_debounce = 0;
	nau8825->xtalk_enable = of_property_read_bool(dev->of_node,
		"nuvoton,crosstalk-enable");
	nau8825->adcout_ds = of_property_read_bool(dev->of_node,
		"nuvoton,adcout-drive-strong");
	ret = of_property_read_u32(dev->of_node,
		"nuvoton,adc-delay-ms", &nau8825->adc_delay);
	if (ret)
		nau8825->adc_delay = 125;
	if (nau8825->adc_delay < 125 || nau8825->adc_delay > 500)
		dev_warn(dev, "Please set the suitable delay time!\n");

	nau8825->mclk = clk_get(dev, "mclk");
