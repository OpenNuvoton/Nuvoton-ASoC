...
static bool nau8325_readable_reg(struct device *dev, unsigned int reg)
{
	...
	case NAU8325_R29_DAC_CTRL1:
	case NAU8325_R2A_DAC_CTRL2:
	case NAU8325_R2C_ALC_CTRL1 ... NAU8325_R2F_ALC_CTRL4:
	case NAU8325_R40_CLK_DET_CTRL:
	case NAU8325_R49_TEST_STATUS:
...
static bool nau8325_volatile_reg(struct device *dev, unsigned int reg)
{
	...
	case NAU8325_R1D_DEBUG_READ1 ... NAU8325_R22_DEBUG_READ3:
...
static int nau8325_osr_apply(struct nau8325 *nau8325)
{
	...
	/* CLK_DAC = OSR * FS
	 * DAC clock frequency is defined as Over Sampling Rate (OSR)
...
proc_done:
	...
	dev_info(nau8325->dev, "nau8325->fs=%d,range=0x%x, %s, (n1,mu,n2,dmu):(%d,%d,%d), MCLK_SRC=%uHz (%d)",
		nau8325->fs,
		(*srate_table)->range,
...
static void nau8325_init_regs(struct nau8325 *nau8325)
{
	...
	if (nau8325->clock_detection)
		regmap_update_bits(regmap, NAU8325_R40_CLK_DET_CTRL,
			NAU8325_CLKPWRUP_DIS | NAU8325_PWRUP_DFT, 0);
	else
		regmap_update_bits(regmap, NAU8325_R40_CLK_DET_CTRL,
