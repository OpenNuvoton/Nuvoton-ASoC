...
#if defined(SPI_4_WIRE_MODE)
static int nau8810_reg_read(void *context, unsigned int reg, unsigned int *value)
{
	struct nau8810 *nau8810 = context;
	struct spi_device *spi = to_spi_device(nau8810->dev);

	int ret;
	u16 reg_buf = cpu_to_be16((0x20 << 8) | (reg << 1));
	u8 val[2];

	ret = spi_write_then_read(spi, (u8 *) &reg_buf, 2 , val, 2);
	if (ret)
	{
		dev_err(nau8810->dev, "nau8810 register read error in 4-Wire Mode\n");
		return -EIO;
	}

	*value = be16_to_cpu((val[1] << 8) | val[0]);

	return 0;
}

static int nau8810_reg_write(void *context, unsigned int reg, unsigned int value)
{
	struct nau8810 *nau8810 = context;
	struct spi_device *spi = to_spi_device(nau8810->dev);

	int ret;
	u16 buf_3 = cpu_to_be16((reg << 9) | value);
	u32 buf_4 = cpu_to_be32((0x10 << 24) | ((reg << 17) | (value << 8)));

	if(nau8810->spi_4_wire_mode)
	{
		ret = spi_write(spi, (u8 *) &buf_4, 3);
		if (ret)
		{
			dev_err(nau8810->dev, "nau8810 register write error in 4-Wire Mode\n");
			return -EIO;
		}
	}
	else
	{
		ret = spi_write(spi, (u8 *) &buf_3, 2);
		if (ret)
		{
			dev_err(nau8810->dev, "nau8810 register write error in 3-Wire Mode\n");
			return -EIO;
		}
	}
...
static int nau8810_probe(struct snd_soc_component *component)
{
#if defined(SPI_4_WIRE_MODE)
	struct nau8810 *nau8810 = snd_soc_component_get_drvdata(component);

	/* Switch to SPI 4-Wire Mode */
	regmap_update_bits(nau8810->regmap, NAU8810_REG_MISCCTL,
			   NAU8810_SPIEN, NAU8810_SPIEN);
	nau8810->spi_4_wire_mode = true;
#endif
	return 0;
}
...
static int nau8810_spi_probe(struct spi_device *spi)
{
	...
	dev_set_drvdata(dev, nau8810);
#if defined(SPI_4_WIRE_MODE)
	nau8810->regmap = devm_regmap_init(dev, NULL, nau8810, &nau8810_regmap_config);
#else
	nau8810->regmap = devm_regmap_init_spi(spi, &nau8810_regmap_config);
#endif
...
static const struct spi_device_id nau8810_spi_id[] = {
	{ "nau8810", 0 },
	{ "nau8812", 0 },
	{ "nau8814", 0 },
	{ }
};
MODULE_DEVICE_TABLE(spi, nau8810_spi_id);
...
static struct spi_driver nau8810_spi_driver = {
	.driver = {
		.name = "nau8810",
		.of_match_table = of_match_ptr(nau8810_of_match),
	},
	.probe = nau8810_spi_probe,
	.id_table = nau8810_spi_id,
};

module_spi_driver(nau8810_spi_driver);

MODULE_DESCRIPTION("ASoC NAU8810 SPI driver");
